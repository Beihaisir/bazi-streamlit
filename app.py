...
def geocode_address(address: str) -> Optional[Dict]:
    ...
    if GEOPY_AVAILABLE:
        try:
            geolocator = Nominatim(user_agent="bazi_master_v5")
            query = address
...
def useful_avoid(strength: str, rs: Dict[str, float]) -> Tuple[List[str], List[str], List[str]]:
    ...
    if strength == "偏强":
        ...
        avoid = ["正印", "偏印", "比肩", "劫财"]
        notes.append("身偏旺，宜泄、宜耗、宜受规矩锻炼；不宜再一味生扶。")
    elif strength == "偏弱":
        useful = ["正印", "偏印", "比肩", "劫财"]
        avoid = ["食神", "伤官", "正财", "偏财", "正官", "七杀"]
        notes.append("身偏弱，宜先得印比扶身；不宜过早重财重责，容易身心被拖住。")
    else:
        useful = []
        avoid = []
        notes.append("身势中和，喜忌要看格局流通和人生应事，不可机械取用。")

    top = max(rs.items(), key=lambda x: x[1])[0]
    notes.append(f"命局最重之象在「{top}」，此象既是天赋，也是人生反复考试之处。")
...
def year_signal(bazi: Dict, year: int, dy: Optional[Dict], useful: List[str], avoid: List[str]) -> Dict:
    ...
    if clash.get(yb) in branches:
        score += 8
        evidence.append("流年冲动原局")
        themes.append("迁移、环境变化、关系转折、方向重选")
    if yb in branches:
        score += 6
        evidence.append("流年伏吟原局")
        themes.append("旧事重来、主题放大、内外压力重复")

    rels = [GOD_TO_RELATION.get(g) for g in gods + dgods if g in GOD_TO_RELATION]
    for r in set(rels):
        if r == "印":
            themes.append("学业、证书、贵人、家庭、房屋、长辈")
        elif r == "比劫":
            themes.append("朋友、竞争、合伙、同辈、人情往来")
        elif r == "食伤":
            themes.append("表达、作品、技术、离职冲动、子女缘")
        elif r == "财":
            themes.append("钱财、项目、客户、感情现实、资产")
        elif r == "官杀":
            themes.append("职位、考试、领导、规则、压力、责任")
    ...
    age = year - int(st.session_state.get("birth_year_for_calc", year))
    importance = abs(score) + (8 if age in [6, 12, 15, 18, 22, 24, 28, 30, 36, 42, 48, 54, 60] else 0)

    return {
        "年份": year, "年龄": age, "流年": ygz, "大运": dgz,
        "分": score, "重要度": importance, "倾向": tone,
        "主题": list(dict.fromkeys(themes))[:3],
        "依据": list(dict.fromkeys(evidence))[:4],
    }


def ten_representative_years(bazi: Dict, birth_year: int, now_year: int, dys: List[Dict], useful: List[str], avoid: List[str]) -> List[Dict]:
    st.session_state["birth_year_for_calc"] = birth_year
    rows = []
    for y in range(birth_year, now_year + 1):
        rows.append(year_signal(bazi, y, current_dayun(dys, y), useful, avoid))
...
def master_temperament(bazi, strength, rs, ts, rng):
    top_rel = max(rs.items(), key=lambda x: x[1])[0]
    second_rel = sorted(rs.items(), key=lambda x: x[1], reverse=True)[1][0]

    if strength == "偏强":
        core = "骨子里不太愿意被人牵着走，遇事有自己的判断，也容易有一股不服输的劲。"
        caution = "但身旺之人最怕困在自己的判断里，听得进逆耳话，格局才会打开。"
    elif strength == "偏弱":
        core = "外界环境对你影响很大，越是大事，越不能孤军硬扛，必须借平台、借人、借规则。"
        caution = "身弱之人不是不能成事，而是不能用硬冲的方式成事，借力就是你的命门。"
    else:
        core = "适应力不差，真正决定高低的不是胆子大小，而是有没有找到合适的路径。"
        caution = "中和之命最怕摇摆，方向一乱，优势也会被消耗掉。"

    return (
        f"性情上，{core}命局里「{top_rel}」重，外在表现多带有{RELATION_MEANING[top_rel]['象']}的味道；"
        f"同时「{second_rel}」也不轻，所以人生不是单线条，常常是一边要处理{RELATION_MEANING[top_rel]['象']}，一边又要面对{RELATION_MEANING[second_rel]['象']}。\n\n"
        f"这里有一句断语：{caution}"
    )


def master_event_text(e, rng):
    themes = "、".join(e["主题"]) if e["主题"] else "阶段转换"
    ev = []
    if "学业" in themes or "证书" in themes:
        ev.append("学业、考试、证书、老师长辈或平台机会有明显牵动")
    if "钱财" in themes or "项目" in themes:
        ev.append("钱财、项目、客户、资产或现实关系出现重要变化")
    if "职位" in themes or "压力" in themes:
        ev.append("工作压力、职位责任、规则考试或领导关系成为主题")
    if "朋友" in themes or "合伙" in themes:
        ev.append("朋友、同辈、合伙、人情往来容易影响选择")
    if "迁移" in themes or "环境" in themes:
        ev.append("居住、学校、工作环境或人生方向可能有变动")
    if not ev:
        ev.append("这一年多主心境、环境或人生节奏的调整")
    ...
    return f"约{e['年龄']}岁，{e['流年']}年。{pick(rng, ev)}。{tone}依据是：{'；'.join(e['依据']) or '流年与原局互动明显'}。"


def master_career(patterns, rs, useful, avoid, rng):
    pnames = [p["名"] for p in patterns]
    top_rel = max(rs.items(), key=lambda x: x[1])[0]
    lines = []

    if "食伤生财" in pnames:
        lines.append("事业不要只求稳定，命里有“把本事变成钱”的路。技术、内容、产品、销售、咨询、培训、自媒体、方案型工作，都比纯执行更有空间。")
    if "官印相生" in pnames:
        lines.append("此命也吃平台和资质，适合在有规则、有门槛、有专业背书的系统里往上走。证书、学历、名分、职位，都不是虚的。")
    if "财生官杀" in pnames:
        lines.append("资源能生责任，项目能换位置。适合做经营、管理、商务、项目制工作，但越做大越要守合同和规则。")
    if "食伤制杀" in pnames:
        lines.append("遇到压力不要硬顶，要拿专业、作品、数据、方案去压住局面。你不是不能碰压力，而是要有本事作为刀。")
    if "比劫夺财" in pnames:
        lines.append("事业上最怕熟人局、兄弟局、口头承诺局。能合作，但钱账和权责必须先小人后君子。")

    if not lines:
        lines.append(f"事业主线落在「{top_rel}」，适合围绕{RELATION_MEANING[top_rel]['象']}去设计路线。")

    lines.append(f"一句话：你的事业不能只看喜欢什么，要看什么能形成闭环。命里最忌“有象无用”，有才要落成产品，有资源要落成现金流，有压力要落成职位或资质。")
    return lines


def master_wealth(ts, useful, avoid):
    wealth = ts["正财"] + ts["偏财"]
    output = ts["食神"] + ts["伤官"]
    bi = ts["比肩"] + ts["劫财"]
    lines = []
    ...
    if wealth >= 1.6:
        lines.append("财星不弱，说明一生不会完全绕开钱、资源、项目、市场这些主题。")
    else:
        lines.append("财星不是最重，求财不能只靠追逐机会，更要靠能力、平台或长期积累来转化。")

    if output >= 1.2:
        lines.append("食伤有气，钱从才华、技术、表达、作品、服务中来，比单纯投机更稳。")
    if bi >= 1.2:
        lines.append("比劫牵财，人情钱、合伙钱、朋友项目要特别慎重，账目不清必有后患。")
    if "正财" in avoid or "偏财" in avoid:
        lines.append("财为风险时，越想快赚，越容易快失；不借钱投资，不替人担保，不碰看不懂的高收益。")
    else:
        lines.append("财若能为用，宜稳步经营客户、渠道、资产，不宜急躁求暴利。")
    return lines


def master_relationship(bazi, gender, avoid):
    ...
    if bad:
        lines.append("关系里最怕因为现实条件、压力、控制欲或责任不对等而委屈自己。感情不能只看喜欢，还要看是否消耗命局关键之气。")
    else:
        lines.append("适合找能共同承担现实、又不压制你发展路径的人。感情若能互相成就，反而能带动事业和心性稳定。")
    return lines


def master_health(es):
    ...
    return [
        f"五行偏弱在「{weak}」，平时要留心{body[weak]}。",
        f"五行偏旺在「{strong}」，旺处也会成过载点，尤其在压力大时要留心{body[strong]}。",
        "健康断语只作体质提醒，不能替代检查和治疗；有症状要看医生。"
    ]


def decision_judgement(decision, useful, avoid, current_signal):
    target = DECISION_TYPES[decision]
    score = 55 + len(set(target) & set(useful)) * 8 - len(set(target) & set(avoid)) * 10
    if current_signal["倾向"] == "顺中有成":
        score += 8
    elif current_signal["倾向"] == "险中有变":
        score -= 8
    score = max(0, min(100, score))
    if score >= 78:
        text = "可以做，但要带着章法做，不是凭热情做。"
    elif score >= 58:
        text = "可以试，但宜小步验证，不宜一次性压上全部筹码。"
    else:
        text = "此事目前不宜重仓，先缓、先查、先留后路。"
    return score, text
...
st.title("☯️ 命理大师详批系统 V5")
st.caption("前台按“大师批命”的方式输出，后台自动排盘、真太阳时、大运流年、十件应事。")
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("附问一事")
    decision = st.selectbox("想顺便问的事", list(DECISION_TYPES.keys()), index=1)
    question = st.text_area("具体问题", value="今年适不适合做副业或创业？", height=90)
...
if use_true_solar:
    ...
    chart_dt = local_dt
    solar_corr = {"经度修正分钟": 0, "均时差分钟": 0, "总修正分钟": 0}
...
cur_signal = year_signal(bazi, CURRENT_YEAR, cur_dy, useful, avoid)
rep_years = ten_representative_years(bazi, int(year), CURRENT_YEAR, dys, useful, avoid)
...
if geo:
    st.caption(f"已按出生地址自动定位，并换算真太阳时。排盘时间：{chart_dt.strftime('%Y-%m-%d %H:%M')}。")
else:
    st.caption(f"出生地址未能自动定位，暂按东八区标准经线处理。排盘时间：{chart_dt.strftime('%Y-%m-%d %H:%M')}。")
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "大师总断", "十件应事", "事业财运", "婚恋健康", "流年与附问", "命盘依据"
])
...
with tab1:
    ...
    st.markdown("### 性情与命中主线")
    st.markdown(master_temperament(bazi, strength, rs, ts, rng))

    st.markdown("### 此命最要紧的三句话")
    key_lines = [
        f"第一，命里最重的是「{max(rs.items(), key=lambda x: x[1])[0]}」，这不是小事，是贯穿一生的主线。",
...
with tab2:
    st.header("出生至今最具代表性的十件应事")
    st.caption("这里不是流水账，而是命理上最容易应验、最该回头核对的十个年份。")

    for i, e in enumerate(rep_years, 1):
        with st.expander(f"{i}. {e['年份']}年，约{e['年龄']}岁：{e['倾向']}", expanded=(i <= 3)):
            st.markdown(master_event_text(e, rng))

    st.markdown("### 校验建议")
    st.write("若这十个年份里，有六七个能对应你的人生大事，此盘取象基本可用；若多数不应，优先检查出生时辰、出生地和真太阳时。")

with tab3:
    st.header("事业与财运")

    st.markdown("### 事业怎么走")
    for line in master_career(patterns, rs, useful, avoid, rng):
        st.write(f"- {line}")

    st.markdown("### 财从哪里来，坑在哪里")
    for line in master_wealth(ts, useful, avoid):
        st.write(f"- {line}")
...
with tab4:
    ...
    st.markdown("### 婚恋关系")
    for line in master_relationship(bazi, gender, avoid):
        st.write(f"- {line}")

    st.markdown("### 健康体质")
    for line in master_health(es):
        st.write(f"- {line}")

    st.markdown("### 生活方式")
    top_rel = max(rs.items(), key=lambda x: x[1])[0]
    st.write(f"你的生活方式要围绕「{top_rel}」来调。{RELATION_MEANING[top_rel]['顺']} 同时也要防：{RELATION_MEANING[top_rel]['逆']}")

with tab5:
    st.header("流年与附问")

    st.markdown("### 当前年份")
    st.write(f"今年为 **{CURRENT_YEAR}年 {year_ganzhi(CURRENT_YEAR)}流年**，当前大运为 **{cur_dy['大运'] if cur_dy else '未识别'}**。")
    st.write(f"今年倾向：**{cur_signal['倾向']}**。")
    st.write(f"主题：{'、'.join(cur_signal['主题']) if cur_signal['主题'] else '阶段调整'}。")
    st.write(f"依据：{'；'.join(cur_signal['依据']) if cur_signal['依据'] else '流年与原局互动较平'}。")

    st.markdown("### 未来十年简批")
    future = []
    for y in range(CURRENT_YEAR, CURRENT_YEAR + 10):
        sig = year_signal(bazi, y, current_dayun(dys, y), useful, avoid)
        future.append({"年份": y, "年龄": y-int(year), "流年": sig["流年"], "大运": sig["大运"], "倾向": sig["倾向"], "主题": "；".join(sig["主题"]), "依据": "；".join(sig["依据"])})
    st.dataframe(pd.DataFrame(future), use_container_width=True, hide_index=True)

    st.markdown("### 附问")
    st.write(f"你问的是：**{question}**")
    dscore, dtext = decision_judgement(decision, useful, avoid, cur_signal)
    st.metric("此事当前可行度", dscore)
...
with tab6:
    ...
    st.download_button(
        ...
        file_name=f"{name}_命理大师详批_V5.json",
